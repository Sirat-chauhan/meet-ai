...
def forgot_password(
    ...
    if is_supabase_auth_enabled():
        try:
            ...
            pass
        return RedirectResponse(f"/login?message={quote_plus(generic_message)}", status_code=302)
...
def reset_password_page(request: Request):
    ...
    if not is_supabase_auth_enabled() and not token:
        return _render_reset_password_page(request, error="Missing or invalid reset link.")
    return _render_reset_password_page(request, token=token)
...
def reset_password(
    ...
    if is_supabase_auth_enabled():
        ...
        if not normalized_access_token:
            return _render_reset_password_page(request, error="Missing or expired recovery session.")
        try:
            update_supabase_password(normalized_access_token, password)
        except SupabaseAuthError as exc:
            return _render_reset_password_page(
                request,
                error=exc.message,
        ...
        return RedirectResponse(
            "/login?message=Password+updated.+You+can+log+in+now.",
            status_code=302,
        )

    normalized_token = (token or "").strip()
    if not normalized_token:
        return _render_reset_password_page(request, error="Missing or invalid reset token.")

    user = db.query(User).filter(User.password_reset_token == normalized_token).first()
    if (
        not user
        or not user.password_reset_expires_at
        or user.password_reset_expires_at < datetime.utcnow()
    ):
        return _render_reset_password_page(request, error="Invalid or expired reset link.")
